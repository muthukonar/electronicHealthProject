...
const DoctorDropdown: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchPatients = async () => {
      try {
        const response = await fetch(`http://localhost:3001/api/patients/${doctorId}`);
...
const handleChoose = useCallback(async (event: React.ChangeEvent<HTMLSelectElement>) => {
  const noteChoice = event.target.value;
  setSelectedNote(noteChoice);

if (noteChoice) {
  try {
    const response = await fetch ('https://odphp.health.gov/myhealthfinder/api/v3/itemlist.json?TopicID=${noteChoice}');
    const result = await response.json();
    setApiData(result);
  } catch (error) {
...
const handleSendNote = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/send-note`, {
      method: "POST",
